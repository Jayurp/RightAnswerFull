    ...
    <Layout>
      <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "center" }}>
        {MenuList.map((menuItem) => (
          <Card
            key={menuItem.name}
            sx={{ maxWidth: "300px", display: "flex", m: 2 }}
          >
            <CardActionArea>
              <CardMedia
                sx={{
              ...
              <CardContent>
                ...
                {!cartItems[menuItem.name] && (
                  <Button
                    onClick={() =>
                      handleOrderClick(menuItem.name, menuItem.price)
                    }
                    variant="contained"
                    color="primary"
                  >
      ...
      {isModalOpen && (
        <CartModal
          ...
          sendOrder={createCart}
        />
      )}
      <div className="cartButtonDiv" style={{display:"flex", justifyContent:"right"}}>
      <SvgButton onClick={() => setIsModalOpen(true)} />
        </div>
    </Layout>
  );
};
