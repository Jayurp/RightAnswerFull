...
app.post("/getMenu", (req, res) => {
  ...
  get(child(starCountRef, req.body.branch_name + "/menu"))
    .then((snapshot) => {
      if (snapshot.exists()) {
...
app.get("/getCurrentOrders", (req, res) => {
  const starCountRef = ref(db);
  get(child(starCountRef, "orders/currentOrders"))
    .then((snapshot) => {
      if (snapshot.exists()) {
        res.status(200).send(snapshot.val());
      } else {
...
function updateOrderData(data) {
  ...
  get(
    child(
      starCountRef,
      "vadodara/order_data/" + data.year + "/" + month + "/" + data.date
    )
  )
    .then((snapshot) => {
      if (snapshot.exists()) {
...
function generateInvoice(doc, table, price, year, month) {
  ...
  doc
    .fontSize(12)
    .text("This is Auto-generated Invoice at " + month + "/" + year, {
      align: "center",
    });
}
